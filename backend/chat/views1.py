...
github='abhi-afk-dev'
mail='abhilashkumarmishra713@gmail.com'
...
async def run_workspace_agent(user_query):
    ...
    github_keywords = ["repo", "code", "branch", "commit", "pr", "pull request", "issue"]
    ...
    # --- MODE SELECTION ---
    if "GITHUB" in intents or (has_github_kw and "WEB" not in intents):
        active_mode = "GITHUB"
    ...
    if active_mode == "GITHUB":
        print("🐙 Launching GitHub Server...")
        command = "npx"
        args = ["-y", "@modelcontextprotocol/server-github"]
        gh_user = os.getenv("GITHUB_USERNAME", "Abhilash713")        
        active_instruction = (
            f"You are Kaito in CODING MODE. Act for GitHub user '{github}'. "
            f"Current Repo Context: When user asks about a repo, assume owner is '{github}'. "
        ...
        allowed_keywords = ["github", "issue", "pull_request", "repo", "file", "search", "commit"]
        ...
        command = str(EXECUTABLE)
        args = []
        active_instruction = f"You are Kaito. Act for {mail}."
                    ...
                    while response.function_calls:
                        for call in response.function_calls:
                            ...
                            # --- REMOTE EXECUTION ---
                                ...
                                if active_mode == "WORKSPACE": 
                                    args["user_google_email"] = USER_EMAIL
                                
                                elif active_mode == "GITHUB":
                                ...
                                try:
                                    ...
                                    tool_output = {"error": str(e), "hint": "Resource not found? Try listing directory contents or checking the branch."}
...
def transcribe_audio(audio_file) -> str:
    try:
        # Save the uploaded file temporarily so Whisper can read it
